...
keyargs_define(dzip_inflate_new)
{
    ...
    retval->stream_parameters.max_chunk_size = args.max_chunk_size ? args.max_chunk_size : 1e6;
...
inline static dzip_size count_dead_output_bytes (dzip_inflate_state * state)
{
    const unsigned char * window_min = state->stream.output.end - state->stream_parameters.window_size;
    
    if (window_min > state->stream.output.begin)
    {
	return window_min - state->stream.output.begin;
    }
    else
    {
	return 0;
...
inline static void prune_output (dzip_inflate_state * state)
{
    size_t dead_bytes = count_dead_output_bytes (state);

    if (dead_bytes > state->stream_parameters.max_chunk_size / 2)
    {
//	log_debug ("inflate output prune %zu", dead_bytes);
	buffer_stream_shift (state->stream.output, dead_bytes);
    }
}
...
void dzip_inflate_release_output (dzip_inflate_state * state, dzip_size size)
{
    ...
    assert (state->stream.output_released >= state->stream.output.shift);
...
inline static dzip_size count_dead_input_bytes (dzip_inflate_state * state)
{
    unsigned char * min = buffer_stream_pointer(state->stream.input, state->stream.bytes_read);

    assert (min >= state->stream.input.begin);
    
    return min - state->stream.input.begin;
}
...
inline static void prune_input (dzip_inflate_state * state)
{
    ...
    size_t dead_bytes = count_dead_input_bytes (state);

    if (dead_bytes > 1e5 && dead_bytes > (size_t)range_count (state->stream.input) / 2)
    {
//	log_debug ("inflate input prune %zu", dead_bytes);
...
bool dzip_inflate_is_full (dzip_inflate_state * state)
{
    range_const_unsigned_char output;

    dzip_inflate_get_output(&output, state);
    
    return (size_t) range_count(output) > state->stream_parameters.max_chunk_size;
}

bool dzip_inflate_is_done (dzip_inflate_state * state)
{
    bool retval = (state->stream.input_is_finished) && (state->stream.input.end == buffer_stream_pointer (state->stream.input, state->stream.bytes_read));

    assert ( !retval || (state->state != STATE_READING_CMD) == state->error);
