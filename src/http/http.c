...
void http_get_getline_end (range_char * line, http_get * get)
{
...
void parse_csv_arg (buffer_string * list, char * arg)
{
    buffer_rewrite (*list);
...
/*    log_debug ("buffer: (%zd) [%.*s]",
	       range_count(stream->buffer),
	       (int) range_count (stream->buffer),
	       stream->buffer.begin);*/
...
bool skip_trailer (http_get * get)
{
    range_char line;
...
bool read_chunk_size (http_get * get)
{
    range_char line;
